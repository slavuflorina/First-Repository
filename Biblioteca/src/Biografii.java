...
public class Biografii extends JFrame implements ActionListener {
	private JButton b1;
	private JButton b2;
	private JButton b3;
	private JButton b4;
		...
		try{
			Connection conn=DriverManager.getConnection("jdbc:mysql://localhost/biblioteca","root","flower");
			 String query="select * from carti";
				...
				while(rs.next()){
					if(rs.getString("categorie").equals("Biografii&Memorii"))
	...
	public Biografii(){
		setTitle( "Carti disponibile in categoria Biografii&Memorii" ); 
		setSize( 1200, 600 ); 
