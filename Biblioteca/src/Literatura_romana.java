...
public class Literatura_romana extends JFrame implements ActionListener {
	private JButton b1;
	private JButton b2;
	private JButton b3;
	private JButton b4;
		...
		try{
			Connection conn=DriverManager.getConnection("jdbc:mysql://localhost/biblioteca","root","fllorina");
			 String query="select * from carti";
				...
				while(rs.next()){
					if(rs.getString("categorie").equals("Literatura romana"))
						comboBoxName.addItem(rs.getString("titlu"));
				}
		}catch(Exception ex){	
		}
	}
	public Literatura_romana(){
		setTitle( "Carti disponibile in categoria Literatura romana" ); 
		setSize( 1200, 600 ); 
