...
if (!isset($_SESSION['id'])) {
    ...
    <div class="center">
        ...
        $genderInfo = array();
        $genderInfo = $users->getInfoGender($idGender);
        $genderName = $genderInfo["genderName"];

        $validate = new Helper();
        if ($validate->isValidGender()) {
            $requestData = $_POST;
            $users->updateGender($requestData, $idGender);
        } ?>

        <body>
        <form method="post" action="">
            <p><span class="error">* required field</span></p>
            Name: <input type="text" name="genderName" value="<?php echo $genderName; ?>">
